...
func getColumns(model interface{}, onlyPrimary bool) (columns []column, values []value) {
	...
	for k := 0; k < structType.NumField(); k++ {
		...
		if len(parsedTag) > 2 { // parsedTag is structured following convention, see `parseTagSetting()`
			isPrimaryKey = true
		}

		// if onlyPrimary is enabled then we get the primary keys values only
...
func parseTagSetting(tags reflect.StructTag) []string {
	// Example:
	//	tag `gorm:"column:pkCol;primaryKey"`

	//	=> [column:pkCol primaryKey]
