...
// The `getBinLogData` function uses Reflection with should impose some performance penalty to the program.
//
// => around 1.2 seconds to process 100k records

func Benchmark_wrapperNonNull(b *testing.B) {

	e, _ := mockRowEventBench(b.N)
	model := &benchNonNullStruct{}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = getBinLogData(e, i, model).(benchNonNullStruct)
		_ = model.Bool
		_ = model.DateTime
		_ = model.Int
		_ = model.String
...
func Benchmark_wrapperNullable(b *testing.B) {

	e, _ := mockRowEventBench(b.N)
	model := &benchNullStruct{}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = getBinLogData(e, i, model).(benchNullStruct)
		_ = model.Bool
		_ = model.DateTime
		_ = model.Int
		_ = model.String
