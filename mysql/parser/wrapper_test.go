...
type wrapperTest struct {
	...
	De     dcm.Decimal    `gorm:"column:de"`   // DECIMAL also maps to decimal.Decimal (when config's useDecimal = true, otherwise it is float64)
...
func (s *wrapperTestSuite) TestReadingNumbers(c *C) {
	...
	case model := <-insertChannel:
		...
		dec, _ := dcm.NewFromString("1111111111111234511189999999987444444444444444444444313.11198")
